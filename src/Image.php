...
use PHPFuser\Enum\ImageAlignPosition;
use PHPFuser\Enum\ImageBorderType;
use PHPFuser\Enum\ImageFit;
use PHPFuser\Enum\ImageFlipDirection;
use PHPFuser\Enum\ImageOrientation;
...
class Image {
    ...
    private static function setText(string $image, string $text, string $size, string $color = "#FFFFFF", int $x = 0, int $y = 0, int $angle = 0, int $width = 0, bool $replace = false): bool {
        ...
        if ($loader instanceof SpatieImage && !empty($text)) {
            ...
            $font = PHPFUSER['DIRECTORIES']['FONTS'] . "bookman.ttf";
