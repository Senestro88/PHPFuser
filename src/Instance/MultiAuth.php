...
class MultiAuth {
    ...
    public function generateKey(): string {
        return (new \PHPFuser\Instance\MultiAuth\FixedBitNotation(5, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ234567', true, true))->encode(random_bytes($this->keylength));
    }
    ...
    public function createBase64Image(string $user, string $hostname, string $issuer, string $key): string {
        return (new \PHPFuser\Instance\MultiAuth\QrCodeImage())->createBase64Image(sprintf('%s@%s', $user, $hostname), $key, $issuer);
    }
    ...
    public function createOutputImage(string $user, string $hostname, string $issuer, string $key): void {
        (new \PHPFuser\Instance\MultiAuth\QrCodeImage())->createOutputImage(sprintf('%s@%s', $user, $hostname), $key, $issuer);
    }
    ...
    public function createImageUrl(string $user, string $hostname, string $issuer, string $key): string {
        return (new \PHPFuser\Instance\MultiAuth\QrCodeUrl())->generate(sprintf('%s@%s', $user, $hostname), $key, $issuer);
    }
    ...
    public function getCode(string $key, ?\DateTimeInterface $time = null): string {
        ...
        $notation = new \PHPFuser\Instance\MultiAuth\FixedBitNotation(5, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ234567', true, true);
